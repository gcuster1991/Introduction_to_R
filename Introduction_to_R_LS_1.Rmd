...
#R as a calculator 
Another great thing about using R is that it can operate as a calcualator. Below we will introduce you to using R as a calculator as well as the assignemnt operator. Each of the problems below highlight operators or functions used to perform simple math. 
```{r}
2+7
9-1
2*12
...
Exercise 3: Using R find the answer to "(9*7)/15 + 6". Assign that to a value called "answer1" and multiple it by 15. What is your final product?
```{r}
...
You can also create your own data frames and fill them with values of your choice. In this next section we will work with some of the different types of values you have encountered and create a dataframe of your own. This is another example of different paths which all lead to the same result. First we will make several vectors and combine them. Next, we will make a dataframe by specifying the deminsions we want. 
...
combined
```
As you can see the combiend dataframe consists of two columns, the first numbers, and the second, alphabetic characters. Look at the types of data in each column. The numbers and dbl (another name for numeric), and the alphas are a factor. Next we will use the seq() function to program a larger dataframe. 
...
#you can see the produce looks exactly like the on you made above
combined2

#Now let's use the seq() function to programe a larger dataframe.
combined3 <- data.frame(numbs=seq(from = 2, to = 100, by = 4), alphas=(rep(c("aa", "bb", "cc", "dd", "ee"), times= 5)))

```
This may look complicated but we can break it apart and make sense of it in a piecewise manner. The first section of the code, "numbs=seq(from = 2, to = 99, by = 3)", creates the first column named "numbs". To do this we say we want a sequence that starts at 2 and goes to 99. The by argument says we want to count by 3s. Simple enough right? Now for the second part. "alphas=(rep(c("aa", "bb", "cc", "dd", "ee"), times= 5))", again we want a column named alphas. However, this time we utlize the rep function, which will repeat whatever you want however many of times you want. If you are confused check the help page. Inside of the rep command "(c("aa", "bb", "cc", "dd", "ee"), times= 5))" we are saying we want a column of letters but only want "aa", "bb", "cc", "dd" and "ee" to appear. We want it to repeat 5 times.  

Exercise 10: Below create a dataframe called "combined4" anyway you would like and copy your code to your word document.
...
Exercise 13: Insert a new chunk of code (don't remember how do do this, see line 58) and write a code that will pull only the first two rows of the columns "age" and "triceps". Copy your final code to your lab handout in the exercise 11 location.

As was shown earlier R can act as a calculator. Whats even better is that math you may have to by hand can be completed with a single function. A couple of examples inlcude calculating the mean and standard devaition of a column or set of values. Below is an example.
```{r}
mean(Pima$diastolic)
sd(Pima$diastolic)
```
