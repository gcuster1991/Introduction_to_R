...
Disclaimer: R will yell at you. Get used to it. It happens all the time. Learn to embrace it and you will be well on your way to solving any problem R can throw at you. Did you notice that the symbol "<-" saves whatever is to the right as a variable named what you have to the left of the symbol "<-". This can be extremely useful when selecting a subset of a data frame or running an analysis. You don't want to have to re-run it every time. By saving it to a new variable you can access it whenever you wish. 
...
combined
```
As you can see the combined data frame consists of two columns, the first numbers, and the second, alphabetic characters. Look at the types of data in each column. The numbers and dbl (another name for numeric), and the alphas are a factor. Next we will use the seq() function to program a larger data frame. 
...
```
This may look complicated but we can break it apart and make sense of it in a piece-wise manner. The first section of the code, "numbs=seq(from = 2, to = 99, by = 3)", creates the first column named "numbs". To do this we say we want a sequence that starts at 2 and goes to 99. The by argument says we want to count by 3s. Simple enough right? Now for the second part. "alphas=(rep(c("aa", "bb", "cc", "dd", "ee"), times= 5))", again we want a column named alphas. However, this time we utilize the rep function, which will repeat whatever you want however many of times you want. If you are confused check the help page. Inside of the rep command "(c("aa", "bb", "cc", "dd", "ee"), times= 5))" we are saying we want a column of letters but only want "aa", "bb", "cc", "dd" and "ee" to appear. We want it to repeat 5 times.  

Exercise 10: Below create a data frame called "combined4" anyway you would like and copy your code to your word document.

#Working with data
In the code below we introduce an easy way to pull out a single column from a data set. Run the code below and take a look at the resulting figures. As you will see, the "$" operator allow you to select a single column. 
```{r}
